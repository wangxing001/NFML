...
def train(**kwargs):
    ...
    # 程序意外中断，从某一个epoch继续训练
    if opt.continue_train:
        state_dict = torch.load(opt.continue_model)['state_dict']
        #state_dict = {k: v for k, v in state_dict.items() \
        #        if not ('reduction' in k or 'softmax' in k)}
        model.load_state_dict(state_dict, False)
        print('load continue model ' + opt.continue_model)   
    ...
    copyfile('./models/networks.py', opt.save_dir+'/networks.py')
    print('\n')
    print('all finished!')
