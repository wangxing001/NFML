...
class ResNetEvaluator:
    ...
    def save_incorrect_pairs(self, distmat, queryloader, galleryloader, 
        ...
        for i in range(m):
            ...
            axes[0].set_title(int(q_pids[i]), color='blue')
            axes[0].imshow(img)
            axes[0].set_axis_off()
            # retrieval image
            for j in range(10):
                ...
                img = Image.open(img).convert('RGB')
                # draw picture
                if g_pids[gallery_index]==q_pids[i]:
                    axes[j+1].set_title(int(g_pids[gallery_index]), color='green')
                else:
                    axes[j+1].set_title(int(g_pids[gallery_index]), color='red')
    ...
    def evaluate(self, queryloader, galleryloader, queryFliploader, galleryFliploader, 
        ...
        if savefig:
            ...
            self.save_incorrect_pairs(distmat.numpy(), queryloader, galleryloader, 
                g_pids.numpy(), q_pids.numpy(), g_camids.numpy(), q_camids.numpy(), './savefig/cuhk03-detected') # 此处的savefig要修改
