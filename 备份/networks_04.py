...
class PBN(nn.Module):
    def __init__(self, input_dim, num_classes, do_reduction=False, num_reduction=512):
        ...
        # self.reduction = nn.Linear(input_dim*2, num_reduction) # 别忘了赋予参数
        # self.reduction.apply(weights_init_kaiming)
        # self.reduction = reduction
        reduction = []
        reduction += [nn.Conv2d(2*input_dim, num_reduction, kernel_size=1, bias=True)]
        reduction += [nn.BatchNorm2d(num_reduction)]
        reduction += [nn.LeakyReLU(0.1)]
    ...
    def forward(self, x):
        ...
        #     x5 = self.reduction(x3) 
        # else:
        #     x5 = x4

        # conv1x1 reduction
        x4 = self.reduction(x3)
        # squeeze 
        x5 = torch.squeeze(x4)
        # BNNeck
        x6 = self.bn(x5)
...
class BFE(nn.Module):
    def __init__(self, num_classes, width_ratio=0.5, height_ratio=0.5):
        ...
        self.PBN1 = PBN(1024, num_classes, do_reduction=True, num_reduction=512)
